...
- The real adapter maps Workbench shield/private-transfer/withdraw actions to the published deposit/transfer/withdraw Wallet API actions. It intentionally rejects a standalone register action because supported wallets register automatically on first STRK20 use.
- Capability detection never reads private balances. Balance reads are a separate consent-gated method. Execution performs a simulated prepare, submits through the wallet, emits normalized lifecycle events, classifies stable errors, and prevents resubmission while a submitted transaction is uncertain.
