...
- [ ] **9. Implement the supported Wallet API adapter and real-mode safety flow**
      Progress: 2026-09-05. The production foundation is implemented: passive wallet discovery, deliberate WalletAccountV6 connection, safe capability metadata, exact deposit/transfer/withdraw action mapping, explicit rejection of fake standalone registration, consent-gated balance reads, simulated prepare-before-submit, stable wallet/prover/submission error mapping, uncertain-transaction idempotency, and an allowlisted server-side RPC receipt verifier. Twenty-three focused wallet/RPC tests pass. Teammate issue #9 supplied the prop-driven readiness panel and was merged after primary review. The homepage now presents the product as a full developer toolkit, labels package and mainnet readiness honestly, and makes the sandbox-to-wallet-to-verified-receipt path understandable within thirty seconds. Teammate issue #11 owns only the developer architecture Documentation route. Remaining work is real-mode UI integration, action review, the second browser journey, and a real supported-wallet smoke.
