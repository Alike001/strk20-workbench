...
- [x] **3. Run the wallet and package compatibility spike before building real-mode UI**
      Completed: 2026-09-06. The exact 10.4.0 starter and 10.5.0 Workbench matrices build successfully. Ready X `5.33.9` connected on Starknet Mainnet, advertised Wallet API `0.10.3`, exposed the required STRK20 methods, returned private balances only after consent, and completed the real Shield → Private transfer → Withdraw lifecycle. The documented upstream questions remain useful for future SDK, devnet, and multi-wallet expansion but do not block the verified Ready X path.
...
- [x] **7. Connect the sandbox to the timeline, privacy X-ray, and review pause 2**
      Completed: 2026-09-05. The participant approved the clearer drop-in STRK20 component-kit direction and official near-black/orange protocol palette. The homepage explains the product in one sentence, previews Alice sending privately to Bob, and introduces Shield/Send privately/Withdraw as reusable building blocks. The playground hides registration as automatic first-use setup, presents three human-readable stages and a single current action, shows before/after balances and plain-language visibility facts, and moves timelines, injected failures, and protocol detail under **Developer details**. The permanent Playwright journey covers the canonical lifecycle, custom amounts, persistence, visible-action failure/retry, reset, and responsive overflow using system Chrome. Desktop 1536×1024 and mobile 390×844 renders were compared with the three accepted concepts and corrected for headline wrapping, vertical density, and mobile navigation overflow.
...
- [x] **8. Prove developer adoption with Integrate documentation and a separate consumer**
      Completed: 2026-09-05. The workspace-only `@strk20-workbench/react` package exports controlled Shield, PrivateTransfer, Withdraw, FlowProgress, and PrivacyFacts components plus the official STRK20 stylesheet. A separate lightweight Vite application imports only public package exports and runs the complete simulated three-stage component flow. Teammate PR #8 supplied the Components page within its assigned boundaries; primary review rebased it onto the package milestone, corrected outdated readiness copy and unapproved labels, verified desktop/mobile fidelity, and merged it only after CI and GitGuardian passed. The page now includes the exact fresh-clone commands and public React API example. A clean shallow clone completed a frozen install and independently built the React package and example with no web-internal imports.
      ...
      What to build: Finalize the `lab-core` public API; add an intentionally small React component package for Shield, Send privately, Withdraw, flow progress, and visibility facts; create the separate `apps/example` consumer using only public package exports; implement the Integrate route with copyable component examples, the shortest install/start path, expected results, supported modes, and compatibility statement; add package README, five-minute quickstart, architecture, privacy-boundary, troubleshooting, and contribution documentation.
      Acceptance: The example is visibly separate from the workbench; it imports no web internals; a React developer can try the private-transfer flow in Sandbox and identify how to install/copy the reusable parts within five minutes; the repository demonstrates reusable STRK20 infrastructure rather than a single hard-coded frontend.
...
- [x] **9. Implement the supported Wallet API adapter and real-mode safety flow**
      Completed: 2026-09-06. The production flow provides passive wallet discovery, deliberate WalletAccountV6 connection, safe capability metadata, exact deposit/transfer/withdraw action mapping, first-use registration recovery, consent-gated balance reads, stable wallet/prover/submission error mapping, uncertain-transaction idempotency, manual hash recovery when the wallet omits a hash, and allowlisted server-side receipt verification. Seven Playwright journeys cover wallet-free fallback, unregistered-account recovery, omitted-hash recovery, canonical sandbox behavior, and responsive layouts. Ready X `5.33.9` completed all three reviewed operations on Starknet Mainnet without the Workbench requesting or storing private material.
...
- [x] **10. Execute and verify the mainnet path, then pass review pause 3**
      Completed: 2026-09-06. The curated evidence set contains one successful Shield, one successful Private transfer, and one successful Withdraw. All three receipts are accepted on Starknet Mainnet, emit an event from the official STRK20 pool, appear in `strk20.json`, and are independently classified as verified by both the deployed Evidence page and the sprint hub. Failed, pending, teammate-registration, and accidental duplicate transactions are excluded. The participant completed the real lifecycle and approved continuing to final pre-demo verification, satisfying review pause 3.
...
- [x] **11. Apply the depth gate, finish quality, and make the product release-ready**
      Completed: 2026-09-07. The optional anonymizer was deferred because the required Wallet API lifecycle already supplies the strongest truthful demo and a late contract addition would add review, deployment, and privacy-claim risk. The release candidate passes 207 unit/component tests, 96.54% statement and 93.25% branch coverage, 12 Playwright journeys, five-route automated accessibility scans, responsive checks, lint, typecheck, formatting, structural metadata validation, both production builds, and a dependency audit with no known vulnerabilities. Next.js is pinned to `16.3.4`; production responses include a restrictive CSP and browser security headers; public pages and APIs return successfully; and the official sprint hub independently reports the live demo plus all three mainnet pool transactions verified.
