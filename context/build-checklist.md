...
- [ ] **9. Implement the supported Wallet API adapter and real-mode safety flow**
      Progress: 2026-09-05. The production foundation and UI flow are implemented: passive wallet discovery, deliberate WalletAccountV6 connection, safe capability metadata, exact deposit/transfer/withdraw action mapping, explicit rejection of fake standalone registration, consent-gated balance reads, simulated prepare-before-submit, stable wallet/prover/submission error mapping, uncertain-transaction idempotency, and an allowlisted server-side RPC receipt verifier. The public Workbench is deployed anonymously at `https://strk20-workbench.vercel.app`, backed by a server-only Starknet Mainnet RPC configuration, and connected to GitHub for automatic deployments. It requires mainnet/API/pool/RPC readiness, then presents a human-readable real-action builder and final review before the wallet prepares, proves, submits, and confirms. The controller prevents duplicate in-flight requests and polls uncertain hashes without resubmission. Teammate PR #14 supplied the reviewed presentation boundary, which is integrated. Remaining work is the complete mocked wallet browser journey and a real supported-wallet smoke.
...
- [ ] **10. Execute and verify the mainnet path, then pass review pause 3**
      Progress: 2026-09-05. Strict repository evidence infrastructure is implemented: curated action metadata, `strk20.json` structural validation, normalized duplicate rejection, final receipt classification, official-pool event verification, explorer projection, a no-store `/api/evidence` route, and a responsive Evidence screen that filters sandbox records. Teammate PRs #16 and #18 supplied the receipt and submission-readiness presentation boundaries; both were reviewed, corrected, and integrated. Local gates pass with 180 tests and five browser journeys. The production deployment and server-only Mainnet RPC configuration are live. Remaining work is three deliberate minimal-value transactions through a supported wallet, independent explorer review, metadata population, and review pause 3.
