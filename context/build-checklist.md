...
- [ ] **9. Implement the supported Wallet API adapter and real-mode safety flow**
      Progress: 2026-09-05. The production foundation is implemented: passive wallet discovery, deliberate WalletAccountV6 connection, safe capability metadata, exact deposit/transfer/withdraw action mapping, explicit rejection of fake standalone registration, consent-gated balance reads, simulated prepare-before-submit, stable wallet/prover/submission error mapping, uncertain-transaction idempotency, and an allowlisted server-side RPC receipt verifier. The Workbench now exposes the hosted real-wallet entry point: installed-wallet choice, deliberate connection, mainnet/API/pool/RPC readiness evaluation, actionable unsupported/no-wallet recovery, and an inspectable capability report while Sandbox remains available. A UI-facing execution controller now enforces review-before-wallet, one in-flight request, normalized prepare/submit/confirm phases, safe failed-action review, and receipt polling that never resubmits an uncertain transaction. Teammate issue #13 owns only the prop-driven real-action review panel. Remaining work is integrating that panel with the controller, the complete mocked wallet browser journey, and a real supported-wallet smoke.
